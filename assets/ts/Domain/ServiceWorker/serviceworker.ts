...
class SagutidServiceWorker {
	CACHE_NAME: string;
	ASSETS_TO_CACHE: string[];
	dynamicPaths: string[];

       constructor() {
	       this.CACHE_NAME = CACHE_NAME;
	       // Unified asset list: all static and critical assets
	       this.ASSETS_TO_CACHE = Array.from(new Set([
		       OFFLINE_URL,
	...
	public async checkManifestChange(manifestHref = '/plugins/system/sagutidloader/assets/manifest.webmanifest') {
		try {
       ...
       public async install(event: ExtendableEvent) {
	       ...
	       event.waitUntil((async () => {
		       ...
		       await Promise.all(this.ASSETS_TO_CACHE.map(async (asset) => {
			       try {
				       const response = await fetchWithTimeout(asset);
				       if (response.ok) {
					       await cache.put(asset, response.clone());
					       successCount++;
					       Logger.Info(`SW: Cached asset: ${asset}`, 'ServiceWorker');
				       } else {
					       failCount++;
	...
	public async activate(event: ExtendableEvent) {
		...
		if (req.mode === 'navigate') {
	       ...
	       if (/\.(css|js|png|jpg|jpeg|gif|svg|woff2?|ttf|eot|ico)$/i.test(url.pathname)) {
		       event.respondWith((async () => {
			       ...
			       const debugMode = (self as any).SAGUTID_CONFIG?.debugMode === true;
			       const fetchAndUpdate = async () => {
				       try {
					       const response = await fetch(req);
					       if (response.ok) {
						       await cache.put(req, response.clone());
						       if (debugMode) Logger.Info(`SW: Updated static asset cache: ${req.url}`, 'ServiceWorker');
					       }
					       return response;
				       } catch (err) {
			       ...
			       if (cached) {
				       if (debugMode) Logger.Info(`SW: Serving cached static asset: ${req.url}`, 'ServiceWorker');
				       fetchAndUpdate(); // Don't await, update cache in background
				       return cached;
			       } else {
	       ...
	       // Runtime asset caching with limits (for other GET requests)
	       if (req.method === 'GET' && url.origin === BASE_ORIGIN) {
		       event.respondWith((async () => {
			       const cache = await caches.open(this.CACHE_NAME);
			       const cached = await cache.match(req);
			       ...
			       try {
				       const response = await fetch(req);
				       if (response.ok) {
					       // Limit cache size to 100 entries (FIFO)
					       ...
					       Logger.Warn(`SW: Network fetch failed for ${req.url}: ${response.status}`, 'ServiceWorker');
					       // Try offline fallback for HTML requests
				       ...
				       Logger.Warn(`SW: Failed to fetch runtime asset: ${req.url}`, 'ServiceWorker');
				       // Try offline fallback for HTML requests
				       if (req.headers.get('accept')?.includes('text/html')) {
					       const offlinePage = await cache.match(OFFLINE_URL);
					       return offlinePage || new Response('Offline', { status: 503 });
	...
	public async message(event: ExtendableMessageEvent) {
		const { data } = event;
	 ...
	 */
	public async collectUrls(sitemapUrl: string, out: string[], limit = 0) {
	 ...
	 * limit: 0 = unlimited
	 */
	public async precacheFromSitemap(limit = 0) {
		try {
			const cache = await caches.open(this.CACHE_NAME);
			const startSitemap = '/index.php?option=com_jmap&view=sitemap&format=xml';
			const urls: string[] = [];
